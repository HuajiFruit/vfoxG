          ...
          <template v-if="systemSdks.length">
            ...
            <div class="sdk-grid">
              <div v-for="sdk in systemSdks" :key="sdk.name" class="sdk-card card-system" @click="openDetail(sdk)">
                ...
                <div class="sdk-card-content">
                  <h3>{{ sdk.name }}</h3>
                  <span class="version-count">{{ sdk.versions?.[0]?.version || 'unknown' }}</span>
                </div>
              </div>
            </div>
          </template>
              ...
              <div class="version-card">
                <div class="version-card-header">
                  <div class="flex-center flex-gap-12">
                    <h3 class="version-title">{{ selectedSdk.versions?.[0]?.version || 'unknown' }}</h3>
                    <span class="system-tag">{{ t('sdk.custom') }}</span>
                  </div>
                </div>
            ...
            <div class="vfox-versions-section">
              ...
              <div v-else-if="unifiedVersions.length" class="versions-grid">
                <div v-for="ver in unifiedVersions" :key="ver.isCustom ? 'sys-' + ver.path : 'vfox-' + ver.version" class="version-card" :class="{ 'is-current': ver.isCurrent }">
                  <div class="version-card-header">
                    <div class="flex-center flex-gap-12">
                      <h3 class="version-title">{{ ver.version }}</h3>
                      <span v-if="ver.isCurrent" class="current-tag">{{ t('sdk.current') }}</span>
                      <span v-if="ver.isCustom" class="system-tag">{{ t('sdk.custom') }}</span>
                      <span v-else class="vfox-tag">vfox</span>
                    </div>
